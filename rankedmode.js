...
const incrementScore = () => {
    scoreGame.innerText = +scoreGame.innerText + 10
}
...
const drawGrid = () => {
    ...
    for (let i = 0; i < numRows; i++) {
        for (let j = 0; j < numCols; j++) {
            ...
            const fillColor = (i + j) % 2 === 0 ? "#432c68" : "#3d285d";
...
const gameOver = (scoreElement) => {
    ...
    finalScore.innerText = scoreGame.innerText;
    canvas.style.filter = "blur(3px)";

    let userLogado = JSON.parse(localStorage.getItem('userLogado'));
    if (+scoreGame.innerText > +userLogado.score) {
        userLogado.score = scoreGame.innerText;
        localStorage.setItem('userLogado', JSON.stringify(userLogado));

        let listaUser = JSON.parse(localStorage.getItem('listaUser') || '[]');
        let userIndex = listaUser.findIndex((user) => user.userCad === userLogado.user);
        if (userIndex !== -1) {
            listaUser[userIndex].scoreCad = userLogado.score;
